...
async function firstSong() {
	await $fetch(`/api/countdowns/${props.countdownCode}/start`, { method: "POST"});
	const data = await $fetch(`/api/countdowns/${props.countdownCode}/next`, { method: "POST"});
...
async function markComplete(): Promise<PlayerVote> {
	const res = await $fetch(`/api/countdowns/${props.countdownCode}/complete`, { method: "POST"});
  return res as any;
}
